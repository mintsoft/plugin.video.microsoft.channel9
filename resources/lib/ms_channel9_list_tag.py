    ...
    def getVideos( self ) :
        ...
        if ul_paging :
            if ul_paging.find( "li", { "class" : "next" } ) :
                ...
                xbmcplugin.addDirectoryItem( handle = int(sys.argv[1]), url = "%s?action=list-tag&tag-url=%s&page=%i" % ( sys.argv[0], urllib.quote_plus( self.tag_url ), self.current_page + 1 ), listitem = listitem, isFolder = True)
