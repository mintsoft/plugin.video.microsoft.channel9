...
class Main:
    ...
    def getVideos( self ) :
        ...
        url              = "http://channel9.msdn.com/%s?page=%u" % ( self.series_url, self.current_page )
        ...
        if ul_paging :
            if ul_paging.find( "li", { "class" : "next" } ) :
                ...
                xbmcplugin.addDirectoryItem( handle = int(sys.argv[1]), url = "%s?action=list-series&series-url=%s&page=%i" % ( sys.argv[0], urllib.quote_plus( self.series_url ), self.current_page + 1 ), listitem = listitem, isFolder = True)
